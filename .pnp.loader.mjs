...
const HAS_BROKEN_FSTAT_FOR_ZIP_FDS =
  major > 25 || (major === 25 && minor >= 7) || (major === 24 && minor >= 15)
...
async function load$1(urlString, context, nextLoad) {
  ...
  const shouldReadSource =
    format === `commonjs` && HAS_BROKEN_FSTAT_FOR_ZIP_FDS && filePath.includes(`.zip/`)
  const source =
    format !== `commonjs` || shouldReadSource
