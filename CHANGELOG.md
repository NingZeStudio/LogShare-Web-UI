...
### 新功能
- **AI 分析排队状态**：适配后端 `ai.queue` 微队列，SSE 首帧 `{"type":"queued","position":N}` 以可展开步骤展示「排队中，前方约 N 个任务」，分析真正开始后自动标记为「排队完成」；队列满返回的 HTTP 429 复用限流文案提示

### 修复
- **首页粘贴超长日志时确认按钮不可达**：编辑器根容器由 `min-h-[calc(100vh-3.5rem)]` 改为确定高度 `h-[calc(100vh-3.5rem)]`，使 textarea 在 flex 链内正确约束并内部滚动，悬浮的「选择文件 / 保存日志」按钮不再被推出可视区、页面滚动与顶栏行为恢复正常

### 文档
- API.md 与站内 API 文档页补充 `queued` 事件、队列模式行为（429 队列满、断连不取消、无排队超时）与 SSE 解析说明
